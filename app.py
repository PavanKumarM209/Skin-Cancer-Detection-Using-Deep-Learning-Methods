...
def send_report_email(patient_email, patient_name, report_filepath, diagnosis=None):
    """Send PDF report to patient's email using SendGrid"""
    try:
        # Check if SendGrid API key is configured
        sendgrid_api_key = os.getenv('SENDGRID_API_KEY')
        from_email = os.getenv('SENDGRID_FROM_EMAIL')
        from_name = os.getenv('SENDGRID_FROM_NAME', 'Skin Cancer Detection System')
        
        if not sendgrid_api_key:
            print("⚠️ Email not configured: SENDGRID_API_KEY not set")
            return False
        
        if not from_email:
            print("⚠️ Email not configured: SENDGRID_FROM_EMAIL not set")
            return False
        
        if not patient_email:
            print("⚠️ Cannot send email: No patient email provided")
            return False
            
        if not report_filepath or not os.path.exists(report_filepath):
            print(f"⚠️ Cannot send email: Report file not found at {report_filepath}")
            return False
        
        # Create email subject with diagnosis if available
        subject = 'Your Skin Cancer Analysis Report'
        if diagnosis:
            subject = f'Your Skin Cancer Analysis Report - {diagnosis}'
...
<head>
    <style>
        ...
        .notice {{ background: #fff3cd; border: 1px solid #ffc107; padding: 15px; border-radius: 5px; margin: 15px 0; }}
...
<body>
    <div class="container">
        <div class="header">
            <h1>Skin Cancer Analysis Report</h1>
        </div>
        <div class="content">
            <p>Dear <strong>{patient_name}</strong>,</p>
            <p>Your skin cancer analysis report has been generated and is attached to this email.</p>
            <p>Please review the report carefully and consult with a healthcare professional if you have any concerns or questions about the results.</p>
            
            <div class="notice">
                <strong>Important Notice:</strong>
                <ul>
                    <li>This report is generated by an AI-powered analysis system</li>
                    <li>It should not be used as a substitute for professional medical advice</li>
                    <li>Please consult with a dermatologist or healthcare provider for proper diagnosis and treatment</li>
                </ul>
            </div>
...
Your skin cancer analysis report has been generated and is attached to this email.

Please review the report carefully and consult with a healthcare professional if you have any concerns or questions about the results.

Important Notice:
- This report is generated by an AI-powered analysis system
- It should not be used as a substitute for professional medical advice
- Please consult with a dermatologist or healthcare provider for proper diagnosis and treatment
...
{from_name}
        ...
        # Read and attach the PDF report
        ...
        if response.status_code in [200, 201, 202]:
            print(f"✅ Report email sent successfully to {patient_email} (Status: {response.status_code})")
            return True
        else:
...
def analyze():
    ...
    try:
        ...
        if 'image' not in request.files:
            print(f"⚠️ No image file in request. Files: {list(request.files.keys())}")
        ...
        try:
            ...
            if email:
                diagnosis = prediction_result.get('top_class', 'Unknown')
                email_sent = send_report_email(email, name, report_filepath, diagnosis)
                response['email_sent'] = email_sent
                if email_sent:
                    print(f"✅ Report emailed to {email}")
                else:
                    print(f"⚠️ Could not email report to {email}")
            else:
...
def patient_login():
    ...
    try:
        ...
        print(f"Error in patient_login: {e}")
        import traceback
        traceback.print_exc()
